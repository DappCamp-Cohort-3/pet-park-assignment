...
describe("PetPark", function () {
  ...
  describe("borrow", function () {
    ...
    it("should revert when pet is already borrowed", async function () {
      ...
      await petPark.add(AnimalType.Dog, 5);

      await petPark.connect(account1).borrow(24, Gender.Male, AnimalType.Fish);

      await expect(
        petPark.connect(account1).borrow(24, Gender.Male, AnimalType.Fish)
      ).to.be.revertedWith("Already adopted a pet");

      await expect(
        petPark.connect(account1).borrow(24, Gender.Male, AnimalType.Dog)
      ).to.be.revertedWith("Already adopted a pet");
    });
    ...
    it("should revert when address details do not match from previous calls", async function () {
      ...
      await expect(
        petPark.connect(account1).borrow(23, Gender.Male, AnimalType.Fish)
      ).to.be.revertedWith("Invalid Age");

      await expect(
        petPark.connect(account1).borrow(24, Gender.Female, AnimalType.Fish)
